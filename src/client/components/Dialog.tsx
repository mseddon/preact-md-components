...
export class Dialog extends Component<{title?: string, buttons?: VNode[], modal?:boolean, show: boolean, onclose?: () => void }, undefined> {   
    render() {
        return <div>
            {this.props.show ? [
                <Portal into="#md-layer-dialog"><div className="md-dialog"><h1>{this.props.title}</h1><div className="md-dialog-content">{this.props.children}</div><div className="right-icons">{this.props.buttons}</div></div></Portal>,
